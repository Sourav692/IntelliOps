...
The LLM-driven core, built on **LangGraph's prebuilt `create_react_agent`**. Single file:

- `agent.py` — exports `build_graph(llm_endpoint, system_prompt, vs_endpoint, vs_index, table_actions, ...)`. Inside the factory:
  - The four tools (`query_features`, `query_system_tables`, `search_knowledge`, `log_action_record`) are defined with `@tool` and close over the config values passed in — no global injection, no dynamic module loaders.
  - `ChatDatabricks(endpoint=…)` is the LLM client.
  - `create_react_agent(llm, tools=[...], prompt=system_prompt)` is the whole graph. LangGraph handles the ReAct loop, tool dispatch, and message accumulation.
- `01_ask_agent.py` — builds the graph, sets a question widget, and calls `graph.invoke({"messages": [("user", question)]})` directly. The graph is the public surface; there's no `ask()` wrapper.

Design choices:
- **Tools are closures, not globals.** `build_graph` is a factory that captures config. Easy to instantiate multiple agents with different configs (e.g. different LLM endpoints) in the same process.
- **No per-turn memory logging in this version.** If callers want to persist conversations, they wrap `graph.invoke` and write to `intelliops.memory.agent_conversation` themselves. We dropped the in-loop hook because it doubled the file size and the dev-loop notebook already exposes the full message trace.
- **One orchestrator agent**, not per-domain.
- The agent never mutates Databricks resources — destructive operations live in `04_tools/` and are triggered only behind the approval gate.
